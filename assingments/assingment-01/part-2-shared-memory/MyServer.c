...
int main(int argc, char** argv) {
    ...
    mem[SIGNAL_INDEX] = CLIENT_KEY;
    printf("now the server is listening to the client.\n\n");

    while(1) {
        if(mem[SIGNAL_INDEX] == SERVER_KEY) {
        ...
        } else if(mem[SIGNAL_INDEX] == CLIENT_KEY) {
            sleep(1);

        // if recieved founded signal
        } else if(mem[SIGNAL_INDEX] == IS_FOUNDED_KEY) {
            printf("The lucky number [%d] is found –the server will be terminated soon.\n\n", luckyNumber);
            break;
        }
    }
